...
- `characters.json`：角色 registry（组合 id → 名字、身份色板、移动人格）。其中 `name` 由提取器从主表写入，是可选数据：字段缺失（旧缓存）或整份缺失时，界面回退为 `sd_<代号>` 展示，不视为错误。重名（同一角色的多个组合变体）时界面以组合 slug 作限定。
...
右栏「表演」页签另有**眼型/口型标签选择器**（表情表的行）：选择即应用，眨眼与说话状态机随即使用所选行的开合格；它与动作库同属检查工具，选择时停用自动演出。

**动作库**是次要的检查工具（右栏「动作」页签）：直接播放单个动作族或单段。动作名不含表情或头顶件配对（配对只存在于编排数据里），所以脸保持默认；点播时自动演出停用（按钮状态可见），需要时手动重开。

## 界面与快捷键

界面是一块**全幅舞台**加三层浮层:画布铺满窗口;左侧「演出台」浮岛收纳角色网格、表演场景与播放控制;右侧「检查面板」浮岛按页签收纳环境 / 光影 / 表演 / 动作 / 诊断;顶部一条工具栏,底部一条等宽状态条。两块浮岛都可以整体收起(顶栏的左右面板按钮),`H` 进入**纯净模式**隐藏全部界面,再按 `H` 或 `Esc` 或右上角召回钮恢复。
...
换角色会通过 `history.replaceState` 把 `unit=` 写回地址栏,刷新与分享都停在同一角色上。快捷键在输入框聚焦时有意不触发;筛选框里按 `Esc` 会清空筛选并把焦点还给页面。
...
| `ui=0` | 以纯净模式启动(全部界面隐藏,`H` 召回)。 |
| `unit=<n>` | 载入指定编号的角色;换角色后地址栏会自动更新这一项。 |
...
本示例处于 **alpha** 阶段：功能仍在演进，外观与交互可能调整。

程序代码以仓库所附许可证发布。部署实例中出现的角色模型、贴图、音频与名称等游戏内容，版权归其原权利方（SEGA Corporation · Colorful Palette 及各授权方）所有；本项目与上述各方无隶属关系，内容仅用于技术研究。
