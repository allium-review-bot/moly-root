...
**The UI sprite atlases share that source.** They are built into the player data too and are fed by the same `player_data` path: the `playerData` section therefore carries two entries, each with its own outcome, so one failing is never read as the other failing. `ui/atlas/` holds the atlas inventory (every atlas, whole texture and sprite's packed placement and authored geometry), the named atlases' whole-texture PNGs and the named sprites' crops; the scale and the crop semantics are in the data contract's `ui/` section.
