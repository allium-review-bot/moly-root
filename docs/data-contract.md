...
**材质 extras 的 keyword 状态**（2026-09-08 起）：每个导出材质的 `extras` 带三键——`validKeywords` / `invalidKeywords` 是 Unity 序列化字符串数组的**原样**（含空数组：空也是状态，不是缺。999 包 1805 材质里 valid 空 637、invalid 空 560、两者全空 554——后者是 shadow pass 材质的真形），`shaderKeywords` 是**解析形** = valid ∪ invalid、valid 在前（即 Unity `Material.shaderKeywords` 属性的语义）。名字刻意**不叫** `keywords`：site 与角色域的 `keywords` 字段只装 valid 半边，重名会诱使消费方把并集当 valid 集。**本游戏序列化里没有 `m_ShaderKeywords` 键**（1805 材质逐一验证为 0；Unity 2021.2 起该状态拆进 valid/invalid 两数组——D67 单记的 `m_ShaderKeywords` 实指这份状态）。当前内容 distinct：valid 17 种（`_DISABLE_DITHER` 1039 / `_RECEIVE_SHADOWS_OFF` 1037 / `_USE_ALPHA_CLIP` 738 / `_ENABLE_MODULE_REFLECTION` 19 / 其余 13 种零星）、invalid 56 种（多为工具生成的 `_ENABLE_*_ON` / `_SHOW_*_ON` 残留）。导出的 glb 材质实例 1862 > 包内 1805：glb 自包含，依赖包的材质随引用多写 57 次。keyword→shader 变体编译语义（multi_compile）不在此契约内——导出的只是开关联状态，1805/1805 已与解密包源 Material 逐项核对相等。
