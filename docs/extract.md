...
**UI 精灵图集与对话 UI 同源。** 它也内置在 player data 里、也由同一个 `player_data` 路径喂入：`playerData` 一节因此有两条，各带各的成败，一条失败不被读成另一条失败。`ui/atlas/` 落图集盘点（每个图集、每张整图、每个 sprite 的打包位置与作者几何）、点名图集的整图 PNG 与点名 sprite 的裁片；规模与裁片的语义见数据契约的 `ui/` 一节。
